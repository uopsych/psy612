...
* If we augment the matrix $\mathbf{X}$ to be an $n\times2$ matrix, in which the first column is filled with `1`'s, we can simplify our regression equation:

$$\mathbf{y} = \mathbf{b}\mathbf{X}$$

* Note: $\mathbf{b}$ is a $1 \times 2$ matrix containing our estimates of the intercept and slope. 

* One property of the residuals is that the the average residual is 0, so we can remove `e` from the equation as well.
...
* To illustrate this, we'll turn back to an [example from 611](https://uopsych.github.io/psy611/labs/lab-8.html#the_data){target=_"blank"}. 
...
## Minihack 2: Calculating confidence intervals{#minihack2}
