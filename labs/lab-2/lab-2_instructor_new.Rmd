...
REMOVE: The line that yields the smallest sum of squared deviations.
...
# we can also set the number of significant digits to use (you probably don't want to do this)
1.5 * 3.0
options(digits = 1)
1.5 * 3.0
options(digits = 7)
...
round(residuals, 10) == round(model$residuals, 10)
```
...
# REMOVE
cor(health$sr_health, fitted(model))^2
```

>**Question:** What does this r-squared value mean (in plain English)?

REMOVE: $R^2$ represents the proportion of variance in Y that is explained by the model. It is literally the square of correlation between the predicted values of Y from the model and the actual values of Y.
...
REMOVE: If the *p*-value is less than the significance level, your sample data provide sufficient evidence to conclude that your regression model fits the data better than the model with no independent variables.
...
stargazer(model, type = "html", out = here("labs", "lab-2", "output", "reg_tbl_sg.html"))
```
...
```{r, eval = FALSE}
sjPlot::tab_model(model, file = here("labs", "lab-2", "output", "reg_tbl_sjp.doc"))
```

# Minihack 1

You are presenting some of your data at a weekly lab meeting. According to the results of a correlational analysis you ran, extraversion is positively associated with happiness. As you are about to progress to the next slide, the lights in the room begin to flicker and frost appears on the corner of your computer screen. Suddenly, the door of the lab room swings open, slamming against the wall. Standing outside is a figure dressed in dark robes. The figure suddenly rises a foot off the floor and glides into the room. They land at the foot of the table and announce that they are Professor Regressor and they are here to test your stats knowledge.

"First question," Professor Regressor intones. "True or false: You would have achieved different results if you ran a univariate regression with standardized variables instead of a correlation?"
...
"Prove it!" exclaims Professor Regressor. They begin to laugh hysterically.

Using the PSY612_Lab2_Data.csv, which is located in the lab-2/data subfolder, conduct a bivariate correlation between extraversion and happiness. Then, conduct a univariate regression, regressing happiness on extraversion and obtain the standardized estimate for the slope. Finally, prove to Professor Regressor that they are equivalent by conducting a series of logical tests showing the equivalence of the estimate (correlation and standardized slope value), their test statistic, and the p value associated with the test statistic (Hint: You can round to 3 digits, which will probably be necessary).
...
"I am no match for one as powerful as you," they say. "Please, you must share with me the secrets of your models..."

You take pity on Professor Regressor. Create two APA-formatted tables using two different methods we covered today. The first table should correspond to the regression from Minihack 1 (regressing happiness on exraversion) and the second should correspond to the regression from Minihack 2 (regressing social support on extraversion).
