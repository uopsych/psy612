...
* Instead of dummy coding, we can instead use **effects coding** to compare individual group means to the overall mean of the dependent variable (i.e. the grand mean). In effects coding, we give a certain group a `1`, the comparison group a `-1`, and then set the unrelated group(s) to zero. In R, we do this in basically the same way as dummy coding, but just change the values we use to set the codes. 
...
-Is CBT effective relative to the overall sample? 
-Is Psychotherapy effective relative to the overall sample?

> **Question:** What would our effects codes be?

>**Answer:** `r data.frame(EC1 = c(1, 0, -1), EC2 = c(0, 1, -1))`
