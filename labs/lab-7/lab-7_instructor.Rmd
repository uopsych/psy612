...
>**Answer:** for a one-unit increase in individualism, we expect a -0.31-unit decrease in the slope of emotion suppression on social support. The value/interpretation of b3 DOES NOT CHANGE because of centering! 
...
* Interpreting what the interaction between emotion suppression and individualism means from the model coefficients alone is rather difficult. It helps a LOT to plot the interaction instead of trying to interpret nuanced coefficients by themselves. What you end up plotting are "simple slopes" so we will discuss what this means first before diving into making plots. 
...
model_int_coefs # view the data frame of coefficients
```
...
social %>% 
  ...
  labs(title = "Effect of emotion suppression on social support \nat low, mid, & high levels of individualism", 
       x = "Emotion suppression (Centered)", y = "Social Support") +
...
>**Answer:** At high levels of individualism there is a negative relationship between emotion suppression and social support. In other words, for people who are more individualistic, suppressing emotion corresponds to less social support. At low levels of individualism/high levels of collectivism, there is a positive relationship between emotion suppression and social support. In other words, for people who are highly collectivistic, suppressing emotion leads to more social support. 
...
```{r}
simple_slopes_df <- import(here("labs/lab-7/data/613_Lab03_simple_slopes.sav")) %>% 
  janitor::clean_names()

test_perf <- simple_slopes_df %>% 
  mutate_all(as.numeric) %>% 
  rename(study_time = time,
         anxiety = anx) %>% 
  mutate(study_time = round(study_time, 2))


write_csv(test_perf, here("labs/lab-7/data/test_perf.csv"))
```
