...
```{r}
model_b <- lm(data = data, claimed_solved ~ honesty + condition)
...
give me trends at different levels of the variable condition. Estimating the slope of honesty. 

categories including non-shredder and shredder, and the slope are -0.85 and -0.08, respectively. Based on the CL, we can tell whether the slope is significantly different from 0. In here, non-shredder is significant. There is an effect of honesty in the non-shredder condition, but there is no effect of honesty in shredder condition. 
...
Whether the two slopes different from each other. The test of the difference between two slopes. Also it is exactly the same as the previous interaction term in model_c.
...
combinations: give the mean for the particular combination.

the differences between two means grouped by the same honesty variable. 
e.g. when honesty equals 1, the differences of means between non-shredder and shredder condition is not significantly different. Honesty has a significant effect on the number of problem solved when honesty is at 3, 4, and 5. 
