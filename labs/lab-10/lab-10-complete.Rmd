...
```{r}
# your code here
print_mean <- function(x, digits = 2) {
  mean_x <- mean(x)
  
  mean_x_rounded <- round(mean_x, digits)
  
  output <- paste("The mean is", mean_x_rounded)
...
get_r2 <- function(data, index, formula){
  ...
  fit <- lm(formula, data = data_subsetted)
...
get_adj_r2 <- function(data, index, formula){
...
# bootstrap r-squared for different models
boot_adj_r2_model1 <- boot(data      = df,     
                           statistic = get_adj_r2, 
                           formula   = happiness ~ extraversion,
                           R         = 1000) 

boot_adj_r2_model2 <- boot(data      = df,     
                       statistic = get_adj_r2, 
                       formula   = happiness ~ extraversion + pos_express,
                       R         = 1000) 

boot_adj_r2_model3 <- boot(data      = df,     
                       statistic = get_adj_r2, 
                       formula   = happiness ~ extraversion + pos_express + soc_sup,
                       R         = 1000) 

boot_adj_r2_model4 <- boot(data      = df,     
                       statistic = get_adj_r2, 
                       formula   = happiness ~ extraversion + pos_express + soc_sup + optimism,
                       R         = 1000) 

boot_adj_r2_model5 <- boot(data      = df,     
                       statistic = get_adj_r2, 
                       formula   = happiness ~ extraversion + pos_express + soc_sup + optimism + exercise,
                       R         = 1000) 

# medians
median(boot_adj_r2_model1$t)
median(boot_adj_r2_model2$t)
median(boot_adj_r2_model3$t)
median(boot_adj_r2_model4$t)
median(boot_adj_r2_model5$t)

# cis
boot.ci(boot_adj_r2_model1, type = "perc")
boot.ci(boot_adj_r2_model2, type = "perc")
boot.ci(boot_adj_r2_model3, type = "perc")
boot.ci(boot_adj_r2_model4, type = "perc")
boot.ci(boot_adj_r2_model5, type = "perc")
```
...
cor1_results <- tibble(r    = cor1$t,
                                  vars = "Extr., Pos. Expr.")

cor2_results <- tibble(r    = cor2$t,
                                  vars = "Extr., Soc. Supp.")

cor3_results    <- tibble(r    = cor3$t,
                                  vars = "Extr., Opt.")

cor4_results   <- tibble(r    = cor4$t,
                                  vars = "Extr., Exercise")

cor_all <- cor1_results %>% 
  rbind(cor2_results) %>% 
  rbind(cor3_results) %>% 
  rbind(cor4_results) %>% 
  group_by(vars) %>% 
