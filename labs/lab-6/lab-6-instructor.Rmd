...
```{r}
df1 <- import("https://raw.githubusercontent.com/uopsych/psy612/master/labs/lab-6/data/df_raw.csv") %>%
  janitor::clean_names()
...
df5 <- import("https://raw.githubusercontent.com/uopsych/psy612/master/labs/lab-6/data/df_out.txt") %>% 
  janitor::clean_names()
...
The same variables (mostly) are contained in each dataset. Let's inspect just the `df1` dataset.
...
```{r}
model1_aug$ID <- c(1:nrow(model1_aug)) # Add a column of ID numbers to the object containing the model residuals
...
```{r}
model4_aug$ID <- c(1:nrow(model4_aug))
...
```{r}
model5_aug$ID <- c(1:nrow(model5_aug))
