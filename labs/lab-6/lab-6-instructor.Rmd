...
You can also plot a histogram of the residuals using the `hist()` function from base R:

```{r}
hist(x      = model1_aug$.resid,
     freq   = FALSE,
     breaks = 20)

curve(dnorm(x, 
            mean = mean(model1_aug$.resid), 
            sd   =   sd(model1_aug$.resid)),
      add = TRUE)
```
...
Obtain a histogram of the residuals for `model3`:

```{r}
hist(x      = model3_aug$.resid,
     freq   = FALSE,
     breaks = 20)

curve(dnorm(x, 
            mean = mean(model3_aug$.resid), 
            sd   =   sd(model3_aug$.resid)),
      add = TRUE)
```
...
Leverage tells us how far observed values are from mean values for a set of IVs (centroid).

#### Mahalanobis Distance

We can describe the leverage for a given value by calculating its Mahalanobis Distance, which reflects the distance between that point and the center of a distribution in multivariate space.

To calculate Mahalanobis Distance, we can use the `outlier()` function from the `{psych}` package.

```{r}
outlier(df6[, c("extraversion", "social_engagement")])
```
...
ggplot(data = model_aug, aes(x = .resid)) + 
  ...
  stat_function(linetype = 2, 
                fun      = dnorm, 
                args     = list(mean = mean(model_aug$.resid), 
                                sd   =   sd(model_aug$.resid))) +
  theme_minimal()
...
```{r}
model_y_out <- lm(happiness ~ extraversion, data = df5)
model_n_out <- df5[-c(134, 149, 161, 165), ] %>% 
...
```{r}
model <- lm(runtime ~ weight + restpulse + runpulse + maxpulse, data = fitness)
```
