...
library(olsrr)     # for creating a standardized residual chart
library(sjPlot)    # for the function plot_model
```
...
Additionally, you can examine a plot of the model's fitted values by the residuals. We can obtain this plot using the function `plot()`, which is included in base R, by give the model as input.

The `plot()` function provides a set of four different diagnostic plots. Let's look at them all for `model1`.
...
```{r}
ggplot(data = model3_aug, aes(x = .resid)) + 
  geom_density(fill = "purple") +
  stat_function(linetype = 2, 
                fun      = dnorm,
                args     = list(mean = mean(model3_aug$.resid), 
                                sd   = sd(model3_aug$.resid)))+
  theme_minimal()
```
...
```{r}
model4_aug$id <- df4$id

ggplot(data = model4_aug, aes(x = id, y = .resid)) + 
  geom_point() +  
  geom_smooth(se = F) +
  geom_hline(yintercept = 0)
```
...
One standard I have seen is that standardized residuals greater than $\pm2$ are worth inspecting further.

### Studentized Residuals

Studentized residuals are calculated by deleting data points from a dataset one by one, fitting models with the new datasets, and calculating the standardized distance between the deleted data points and the predicted values for the models without the data points.

Let's plot the studentized residuals by using the `ols_plot_resid_stud()` function from the `olsrr` package.
...
One standard I have seen is that studentized residuals greater than $\pm3$ are worth inspecting further.
...
sort(desc(abs(dfbetas[,1])))
sort(desc(abs(dfbetas[,2])))
```

One standard I have seen is that DFBETAS larger than $\frac{2}{\sqrt{n}}$ should be investigated as potential outliers.
...
One standard I have seen is that DFFITS larger than $2 \times \frac{p+1}{\sqrt{n−p−1}}$ should be investigated as potential outliers (n = number of observations, p = number of predictors including the intercept).
...
When there are multiple predictors in a model, the output of the `plot_model()` function from the `{sjPlots}` package function will provide a plot of VIFs (as well as a number of other useful plots).

```{r}
# for just the vif plot
plot_model(model6, type = "diag")[1]

# for all of the useful plots
plot_model(model6, type = "diag")
```

We can also examine the tolerance and VIF values by using the `ols_vif_tol()` function from the `{olsrr}` package.
...
```{r}
model_w_out  <- lm(happiness ~ extraversion, data = df5)
model_no_out <- df5[-c(134, 149, 161, 165), ] %>% 
