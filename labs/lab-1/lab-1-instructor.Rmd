...
In today's lab, we will discuss how to use R to visualize correlations, calculate correlation coefficients, and assess statistical significance of correlations. You can download the Rmd file [here](lab-1.Rmd) to follow along. And [here](lab-1-instructor.Rmd) is the Rmd file for the instructor copy (so you can see inline code).
...
Scatter plots allow us to visualize the relationship between two variables. Here is the code for a scatter plot using `ggplot()`. Add a line of best fit by adding a `geom_smooth()` layer to the plot. 
...
ggplot(data = mtcars, aes(x = mpg, y = hp)) +
  ...
  theme_classic()+
  labs(title = "Relation between mpg and hp")
```
...
Covariance captures how the variances of two variables are related, i.e., how they *co*-vary. If higher values of one variable tend to correspond with higher values of the other variable, and lower values of one variable tend to correspond with lower values of the other variable, then the covariance would be positive. However, if the two variables are inversely related (i.e., higher values on one variable correspond with lower values on another variable), then the covariance would be negative.  
...
ggplot(data = health, aes(x = consc, y = sr_health)) +
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(x = "Conscientiousness", y = "Self-reported health", title = "Relation between conscientiousness and self-reported health")+
  theme_classic()
```
...
To review the steps of calculating confidence intervals using Fisher's transformation, see [here](https://uopsych.github.io/psy612/lectures/1-correlation.html#45){target="_blank"}.
...
crit_t = qt(p = .975, df = n-2, lower.tail = T)
...
b. Visualize the correlations using a SPLOM plot and/or a heat map.
