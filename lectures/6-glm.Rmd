...
The majority of our models fall under the umbrella of a general(ized) linear model
  ...
  + The **general linear model** is a family of models that assume the relationship between your DV and IV(s) is linear and additive, and that your residuals* are normally distributed. 
...
- Each individual (intersection) has a unique Y value, X value, and residual
...
- Unless you've collected a new dataset, you can't really assess the accuracy of preditions. Instead, we evaluate our $\hat{Y}_i$'s instead. We want $\hat{Y}_i$ to be close to our actual data for each person $(Y_{i})$
