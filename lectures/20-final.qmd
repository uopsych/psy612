...
Sometimess we find that the semi-partial correlation squared is larger than the zero-order.

```{r}
data = read.csv("data/vals.csv")
...
mm_drug    = as.data.frame(emmeans(model, ~drug)) %>% 
  select(drug, drug_mean = emmean)
mm_therapy = as.data.frame(emmeans(model, ~therapy)) %>% 
  select(drug, therapy_mean = emmean)
mm_int     = as.data.frame(emmeans(model, ~drug*therapy)) %>% 
  select(drug, therapy, cell_mean = emmean)
...
.purple[Overfitting:] mistakenly fitting sample-specific noise as if it were signal - OLS models tend to be overfit because they minimize error for a specific sample

.purple[Bias:] systematically over or under estimating parameters .purple[Variance:] how much estimates tend to jump around\
