...
The interaction of two or more categorical variables in a general linear model is formally known as [Factorial ANOVA]{style="color:purple"} .
...
The [marginal mean differences]{style="color:purple"} correspond to main effects. They tell us what impact a particular factor has, ignoring the impact of the other factor. 
...
The remaining effect in a factorial design, and it primary advantage over separate one-way ANOVAs, is the ability to examine [simple main effects]{style="color:purple"}, sometimes called conditional effects. 
...
model.data = plot_model(model, type = "pred", terms = c("Speed", "Noise"))$data
...
```{r, message = F, warning = F, fig.width = 5, fig.height = 5,echo =F}
model.data %>% ggplot(aes(x = x, y = predicted, fill = group)) +
  geom_bar(stat = "identity", position = position_dodge(width = .8), color = "black", width = .7) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = .3, position = position_dodge(width = .8)) +
  scale_x_continuous("Speed Condition", breaks = c(1, 2,3), labels = c("Slow", "Medium", "Fast")) +
...
[The speed condition means are clearly different in each noise condition, but the pattern of those differences is not the same]{style="font-size:80%"}

[The marginal speed condition means do not represent well the means in each noise condition.]{style="font-size:80%"}

[An interaction.]{style="font-size:80%"}
...
[The two main effects and the interaction represent three independent questions we can ask about the data. We have three null hypotheses to test.]{style="font-size:80%"}

[One null hypothesis refers to the marginal row means.]{style="font-size:80%"}
...
One part will represent variability [within groups]{style="color:purple"} .  This within-group variability is variability that has nothing to do with the experimental conditions (all participants within a particular group experience the same experimental conditions). 

The other part will be [between-group variability]{style="color:purple"} .  This part will include variability due to experimental conditions.  We will further partition this between-group variability into parts due to the two main effects and the interaction.
...
$$
\begin{aligned}
 SS_{\text{total}} &= \sum_{r=1}^R\sum_{c=1}^C\sum_{i=1}^{N_{rc}}(Y_{rci}-\bar{Y}_{...})^2 \\
 SS_{\text{Within}} &= \sum_{r=1}^R\sum_{c=1}^C\sum_{i=1}^{N_{rc}}(Y_{rci}-\bar{Y}_{rc.})^2 \\
 SS_R &= CN\sum_{r=1}^R(\bar{Y}_{r..}-\bar{Y}_{...})^2\\
SS_C &= RN\sum_{c=1}^C(\bar{Y}_{.c.}-\bar{Y}_{...})^2\\
 SS_{RC} &= N\sum_{r=1}^R\sum_{c=1}^C(\bar{Y}_{rc.}-\bar{Y}_{r..}-\bar{Y}_{.c.}+\bar{Y}_{...})^2 \\
\end{aligned}
$$
...
$$
\begin{aligned}
 F_R &= \frac{MS_R}{MS_{within}} \\
\\
 F_C &= \frac{MS_C}{MS_{within}} \\
\\
 F_{RxC} &= \frac{MS_{RxC}}{MS_{within}} \\
\end{aligned}
$$
...
eta.tab[,c("source", "SS", "eta.sq", "eta.sq.part")] %>%
  kable(., digits = 2,  format = "html",
        col.names = c("Source", "SS", "eta 2", 'partial eta 2'), 
...
[If the design is balanced:]{style="color:purple"}
...
eta.tab[,c("source", "SS", "eta.sq", "eta.sq.part")] %>%
  kable(., digits = 2,  format = "html",
        col.names = c("Source", "SS", "eta2", 'partial eta2'), 
         row.names = F, escape = F) %>%
  column_spec(3, width = "10em")
```
...
In `sjPlot()`, predicted values are the expected value of Y given all possible values of X, [at specific values of M]{style="color:purple"} . If you don't provide specific values of M, you'll get a plot with all possible values.


```{r pred2.plot, message = F}
#| code-fold: true
library(sjPlot)
...
In `sjPlot()`, predicted values are the expected value of Y given all possible values of X, [at specific values of M]{style="color:purple"} . If you don't specify M at all, it will choose the reference group of M.
...
In `sjPlot()`, estimated marginal means are the expected value of Y given all possible values of X, [ignoring M]{style="color:purple"} . 
...
$$
\begin{aligned}
 SE_{\text{mean}} &= \sqrt{\frac{MS_{within}}{N}}\\
 SE_{\text{mean difference}} &= \sqrt{MS_{within}[\frac{1}{N_1}+\frac{1}{N_2}]} \\
 SE_{\text{mean difference}} &= \sqrt{\frac{2MS_{within}}{N}} \\
\end{aligned}
$$
...
Interpretation of a main effect requires comparisons among the [marginal means]{style="color:purple"} .

Interpretation of the interaction requires comparisons among the [cell means]{style="color:purple"} .
...
A [(marginal) main effect]{style="color:purple"} is the effect of IV1 on the DV ignoring IV2.

A [simple main effect]{style="color:purple"} is the effect of IV1 on the DV at a specific level of IV2
...
A [marginal effect]{style="color:purple"} (MEM or AME) is the typica effect of IV1 at the mean of the other variable or across the sample (respectively).

A [simple slope]{style="color:purple"} is the effect of IV1 on the DV assuming IV2 is a specific value.
:::
::::
