---
title: "Power & 3-way interactions"
format: 
  revealjs:
    multiplex: true
    slide-number: true
...
[Regression equation]{style="color:purple"}

$$\hat{Y} = b_{0} + b_{1}X + b_{2}Z + b_{3}W + b_{4}XZ + b_{5}XW $$
$$ + b_{6}ZW + b_{7}XZW$$
...
It is equally correct to say that the magnitude of the A x C interaction depend on levels of B. Or, that the magnitude of the B x C interaction depends on levels of A. These are known as [simple interaction effects]{style="color:purple"}.
...
Especially with small samples, three-way interactions may be the result of a few outliers skewing a regression line. If you have stumbled upon a three-way interaction during exploratory analyses, [be careful.]{style="color:purple"} This is far more likely to be a result of over-fitting than uncovering a true underlying process.
