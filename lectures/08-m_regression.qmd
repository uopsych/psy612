...
- predicted change in $Y$ for a 1 unit change in $X$, <span style="color:purple">holding all other predictors constant</span>
...
Note that significance of partial effects is identital. 
...
[^*]: Actually, standardized regression coefficients are not bounded by -1 and 1, but it's rare to see values this large, and [usually indicative of some multicollinearity](http://www.statmodel.com/download/Joreskog.pdf).
...
Regression coefficient is most similar to the semi-partial correlation coefficient. 
:::

-------------------------------------------------------------------

See here how the estimates differ from one another: 
...
#extract partial slope for give support
round(coef(mod0)["z_give_support"],3)
```
...
```{r, message=F, warning = F, echo = 1:3}
...
Highlight intercept value: what does this mean?
...
Next, how to interpret those slope values? 
...
<span style="font-size:80%">Note that the _p_-values in this table do NOT match the _p_-values in the regression summary table. For the time being, just know that, in a multiple regression framework, you should use the `summary()` output to interpret the unique contribution of predictors, not the `anova()` output. We'll return to these calculations later in the term.</span>

-------------------------------------------------------------------

```{r, output.lines = 12:13}
summary(mr.model)
```
