...
[Warning:]{style="color:purple"} It should be noted that polynomials are often a poor approximation for a non-linear effect of X on Y. However, correctly testing for non-linear effects usually requires (a) a lot of data and (b) making a number of assumptions about the data. Polynomial regression can be a useful tool for exploratory analysis and in cases when data are limited in terms of quantity and/or quality.
...
interdependence determines when more talent is too much or not enough.](https://www8.gsb.columbia.edu/cbs-directory/sites/cbs-directory/files/publications/Too%20much%20talent%20PS.pdf) _Psychological Science 25_(8), 1581-1591.]{style="font-size:80%"}
...
#slope @ 0
slope_val = coef(mod2)["talent"]
intercept_val = predict(mod2, data.frame(talent = 0))
plot_model(mod2, type = "pred", terms = c("talent[all]")) +
  geom_abline(aes(intercept = intercept_val, slope = slope_val), color = "blue")
```
...
```{r, echo = FALSE, warning= FALSE}
#slope @ 50
slope_val = coef(mod2_50)["talent_c50"]
intercept_val = predict(mod2_50, data.frame(talent_c50 = 0))
plot_model(mod2_50, type = "pred", terms = c("talent_c50[all]")) +
  geom_abline(aes(intercept = intercept_val, slope = slope_val), color = "blue")
```
...
```{r, echo = FALSE, warning= FALSE}
# slope val at mean
slope_val = coef(mod2_c)["talent_c"]
intercept_val = predict(mod2_c, data.frame(talent_c = 0))
...
#| code-fold: true
#slope 
slope_val1 = coef(mod2)["talent"] + 2*coef(mod2)["I(talent^2)"]*10
intercept_val1 = predict(mod2, data.frame(talent = 10)) - slope_val1*10

slope_val2 = coef(mod2)["talent"] + 2*coef(mod2)["I(talent^2)"]*70
intercept_val2 = predict(mod2, data.frame(talent = 70)) - slope_val2*70

plot_model(mod2, type = "pred", terms = c("talent[all]")) +
  geom_abline(aes(intercept = intercept_val1, slope = slope_val1), color = "blue") +
  geom_abline(aes(intercept = intercept_val2, slope = slope_val2), color = "red")
```
...
```{r, message = F, fig.width = 10, fig.height=6, warning = F}
#| code-fold: true
library(ggpubr)
mu = mean(heights)
