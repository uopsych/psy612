...
Both methods of specifying the interaction above will work in R. Using the `*` tells R to create both the main effects and interaction effect. Note, however that the following code *gives you the wrong results*
...
In some ways, the interpretation of coefficients in the regression stays the same. The intercept continues to represent the best guess of Y when all predictors in the model are equal to 0 (which may or may not be a meaningful situation). The other coefficients represent the effect of the relative variable holding all the others constant. 

What differs is that you have a new term in the model, which represents the product of two independent variables. So your interpretation of coefficients requires holding this value constant as well. When you add products to the model, you have to change your interpretation of other variables:

$b_1$ is now the effect of X on Y when Z is equal to 0.
...
Often we graph the simple slopes as a way to understand the interaction. hile it's technically not impossible to interpret an interaction from the coefficient terms alone, it requires a lot of calculation and mental rotation to understand the shape of the interaction. For those reasons, consider it a requirement that you graph interactions in order to interpret them. 
...
Recall that the slope at any particular value is a combination of both b1 and b3 
$$\large \hat{Y} = (b_{0} + b_{2}Z) + (b_{1} + b_{3}Z)X$$
$$\large se_{b@z} = \sqrt{se_{b1}^2 + (2 * Z * cov_{b1b3})+ (Z^2 se_{b3}^2)}$$
...
stress.data = stress.data %>%
  mutate(zStress = z_score(Stress), 
         zAnxiety = z_score(Anxiety), 
         zSupport = z_score(Support))
